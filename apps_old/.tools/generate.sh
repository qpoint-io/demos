...
QPOINT_LOCAL_SERVICE=$(cat "$SCRIPT_DIR/qpoint-service.yml")
...
$service_definition
$QPOINT_LOCAL_SERVICE" > "${service_name}/docker-compose.yml"
    # Increment the port
    port=$((port + 1))
  fi
done
...
# Add the qpoint.local service
DOCKER_COMPOSE_CONTENT="$DOCKER_COMPOSE_CONTENT
$QPOINT_LOCAL_SERVICE"
